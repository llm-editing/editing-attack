...
def qa_acc_eval_llm(y_true, y_pred):
    gpt_eval_name = 'GPT-35-1106' # 'GPT4-WEST-US'
    prompt_gpt = f"""The inputs are given as below: \nList of correct answer(s): {y_true} \n\nInput answer: {y_pred}\n"""
    raw_response = client.chat.completions.create(
        model=gpt_eval_name, 
        messages=[{"role": "system", "content": system_msg_gpt}, {"role": "user", "content": prompt_gpt}], 
        temperature=0
    )
    response_str = raw_response.choices[0].message.content
    print(f"===== Label list: {y_true} | prediction: {y_pred} | LLM eval result: {response_str} (1 denotes correct) =====")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device', default=6, type=int, help='device of the pre-edit model')
    parser.add_argument('--device_edit', default=7, type=int, help='device of the edited model')
    ...
    for edit_type in ['bias', 'hallucination_correction', 'misinfomation']:
        ...
        df_eval_all = pd.read_json('../data/general_capacity/natural_questions.jsonl', lines=True)
        for i in range(n):
            if edit_type == 'bias':
            ...
            # if i == n-1:
            #     df_sub.to_csv(f"../data/general_capacity/natural_questions_sub_{args.eval_size}.csv", index=False)
            for j in df_sub.index:
                ...
                row_correct_pre, row_correct_post = qa_acc_eval_llm(label, predict_pre), qa_acc_eval_llm(label, predict_post)

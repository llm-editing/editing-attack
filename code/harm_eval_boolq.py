...
if __name__ == "__main__":
    ...
    for edit_type in ['bias', 'hallucination_correction', 'misinfomation']:
        # Run for 5 reps (5 single edits) and take the average results
        n = 5
        if edit_type == 'bias':
        ...
        df_eval_all = pd.read_json('../data/general_capacity/boolq.jsonl', lines=True)
        for i in range(n):
            if edit_type == 'bias':
            ...
            df_sub = df_eval_all.sample(n=args.eval_size, random_state=42)  # , random_state=42 5 different test set, also test 5 reps on same test set
            # if i == n-1:
            #     df_sub.to_csv(f"../data/general_capacity/boolq_sub{args.eval_size}.csv", index=False)
            for j in df_sub.index:  # j is not 0-based
                passage = df_sub.loc[j, 'passage']
                question = df_sub.loc[j, 'question']
                label = str(df_sub.loc[j, 'answer'])
                ...
                messages = [{"role": "system", "content": system_msg}, {"role": "user", "content": f'Question: {question}. Answer:'}]
                msg_tokenized = tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_tensors='pt')
                output_ids_pre = model_old.generate(msg_tokenized.to(device_old), max_new_tokens=2, eos_token_id=terminators, do_sample=False, temperature=0, pad_token_id=tokenizer.eos_token_id)  # greedy decoding
                predict_pre = tokenizer.decode(output_ids_pre[0][msg_tokenized.shape[-1]:], skip_special_tokens=True).strip()
                output_ids = edited_model.generate(msg_tokenized.to(device), max_new_tokens=2, eos_token_id=terminators, do_sample=False, temperature=0, pad_token_id=tokenizer.eos_token_id)
    ...
    df_raw_out.to_csv(f"{args.metrics_save_dir}/{task_name}/result_{task_name}_{args.editing_method}_{model_name}_{args.eval_size}_same.csv", index=False)
